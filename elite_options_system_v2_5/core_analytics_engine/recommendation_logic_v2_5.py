...
class RecommendationGeneratorV2_5: # Renamed
    ...
    def _initialize_settings_and_mappings(self) -> None:
        ...
        self.signal_to_category_map = self.reco_config.get("signal_to_category_mapping", {})
    ...
    def _apply_gib_conviction_mod(self, signal_name_full: str, gib_value: Optional[float]) -> Tuple[float, Optional[str]]:
        ...
        mod = 0.0; rationale = None; cfg = self.reco_config.get("conviction_modifiers", {}) # More direct path
        ...
        pos_gib_thresh = float(cfg.get("positive_gib_thresh", 15e9))
        neg_gib_thresh = float(cfg.get("negative_gib_thresh", -15e9))
        very_neg_gib_thresh = float(cfg.get("very_negative_gib_thresh", -60e9))

        if is_bullish:
            if gib_value > pos_gib_thresh: mod = float(cfg.get("positive_gib_bullish_mod", 0.5))
            elif gib_value < very_neg_gib_thresh: mod = float(cfg.get("very_negative_gib_bullish_mod", -1.25))
            elif gib_value < neg_gib_thresh : mod = float(cfg.get("negative_gib_bullish_mod", -0.5))
        elif is_bearish:
            if gib_value < neg_gib_thresh: mod = float(cfg.get("negative_gib_bearish_mod", 0.5))
            elif gib_value > pos_gib_thresh: mod = float(cfg.get("positive_gib_bearish_mod", -0.5))
    ...
    def _apply_nvp_conviction_mod(self, signal_name_full: str, nvp_strike_val: Optional[float]) -> Tuple[float, Optional[str]]:
        ...
        mod = 0.0; rationale = None; cfg = self.reco_config.get("conviction_modifiers", {})
        if nvp_strike_val is None or pd.isna(nvp_strike_val): return mod, rationale
        is_bullish = "bullish" in signal_name_full.lower(); is_bearish = "bearish" in signal_name_full.lower()

        confirm_pos = float(cfg.get("nvp_confirm_positive_thresh", 25e6))
        confirm_neg = float(cfg.get("nvp_confirm_negative_thresh", -25e6))
        oppose_pos_for_bearish = float(cfg.get("nvp_oppose_positive_for_bearish_thresh", 10e6))
        oppose_neg_for_bullish = float(cfg.get("nvp_oppose_negative_for_bullish_thresh", -10e6))
        nvp_confirms_mod = float(cfg.get("nvp_confirms_mod", 0.75))
        nvp_opposes_mod = float(cfg.get("nvp_opposes_mod", -1.0))
        ...
        for signal_cat_key, signals_by_type_map in generated_signals_dict_from_sg.items():
            for signal_type_key, list_of_signal_payloads in signals_by_type_map.items():
                for raw_signal_data_payload in list_of_signal_payloads:
                    ...
                    conv_mods_cfg = self.reco_config.get("conviction_modifiers", {})
                    ...
                    min_stars_required = int(self.reco_config.get("star_thresholds", {}).get(min_stars_key_config, 2)) # More robust path
